...
from open_spiel.python.games.tiandijie.primitives.skill.Skill import SkillTargetTypes, is_normal_attack
...
def get_element_advantage_multiplier(attacker_instance: Hero, context: Context) -> float:
    return get_level2_modifier(attacker_instance, ma.element_advantage_multiplier, context)


def get_element_disadvantage_multiplier(attacker_instance: Hero, context: Context) -> float:
    return get_level2_modifier(attacker_instance, ma.element_disadvantage_multiplier, context)


def get_attacker_penetration(attacker_instance, is_magic: bool, context: Context):
    attr_name = ma.magic_penetration_percentage if is_magic else ma.penetration_percentage
    return get_level2_modifier(attacker_instance, context, attr_name)
...
def get_attacker_hit_probability1(attacker_instance: Hero, context: Context) -> float:
    critical_stones_percentage_modifier = accumulate_attribute(attacker_instance.stones.percentage,
    ...
    total_critical = total_luck / 10 + get_level2_modifier(attacker_instance, context,
                                                           ma.critical_percentage) + critical_stones_percentage_modifier
...
def get_attacker_hit_probability(attacker_instance: Hero, context: Context) -> float:
    ...
    level2_critical_modifier = get_level2_modifier(attacker_instance, context, ma.critical_percentage)
...
def get_defender_hit_resistance(defender_instance: Hero, context: Context) -> float:
    # Calculate buffs
    level_2_hit_resistance = get_level2_modifier(defender_instance, context, ma.critical_percentage_reduction)
