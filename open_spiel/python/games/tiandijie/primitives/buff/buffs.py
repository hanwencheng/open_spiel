...
class BuffTemps(Enum):
    ...
    wucui_fengmai = BuffTemp('wucui_fengmai', BuffTypes.Harm, False, False, False, {'passives_disabled': True})

    # 迟缓I	有害	可驱散	可扩散	不可偷取	移动力-1，无法护卫
    chihuan_1 = BuffTemp('chihuan', BuffTypes.Harm, True, True, False, {'absolute_defense_range': 1, 'move_range': -1}, True, 1)

    # 迟缓II	有害	可驱散	可扩散	不可偷取	移动力-2，无法护卫
    chihuan_2 = BuffTemp('chihuan', BuffTypes.Harm, True, True, False, {'absolute_defense_range': 1, 'move_range': -2}, True, 2)

    # 迟缓III	有害	可驱散	可扩散	不可偷取	移动力-3，无法护卫
    chihuan_3 = BuffTemp('chihuan', BuffTypes.Harm, True, True, False, {'absolute_defense_range': 1, 'move_range': -3}, True, 3)

    # 封劲	有害	可驱散	可扩散	可偷取	主动绝学射程-1
    fengjing = BuffTemp('fengjing', BuffTypes.Harm, True, True, True, {'attack_range', -1})

    # 幽霜	其他	不可驱散	不可扩散	不可偷取	免伤+20%，主动造成伤害后，对目标造成1次「固定伤害」（（物攻+物防）的30%），并施加「迟缓I」状态，持续2回合
    youshuang = BuffTemp('youshuang', BuffTypes.Others, False, False, False, {'magic_damage_reduction': 20, 'damage_reduction': 20}, False, 1, [
    ...
    # 禁闭
    jinbi = BuffTemp('jinbi', BuffTypes.Harm, False, False, False, {'action_disabled': True, 'no_counterattack': True}, False, 1, [EventListener(EventTypes.damage_end, 1, partial(Effects.add_partner_harm_buffs, buff_number=2, range=2, duration=2))])

    # 三昧真火	有害	可驱散	不可扩散	不可偷取	法防-15%，行动结束时，遭受1次法术伤害（施加者法攻的50%）
    sanmei_zhenhuo = BuffTemp('sanmei_zhenhuo', BuffTypes.Harm, True, False, False, {'magic_defense': -15}, False, 1, [EventListener(EventTypes.action_end, 1, partial(Effects.take_magic_damage, multiplier=0.5))])
